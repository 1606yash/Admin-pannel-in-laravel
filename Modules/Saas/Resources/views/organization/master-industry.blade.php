...
<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            ...
            @endphp
            <p>You have total <span id="listCount">{{ count($industry) }}</span> {{ $count }}.</p>
...
<div class="nk-block table-compact buyer">
    ...
    <div class="nk-add-product toggle-slide toggle-slide-right" data-content="addIndustriesMaster" data-toggle-screen="any" data-toggle-overlay="true" data-toggle-body="true" data-simplebar id="modal">
        <form role="form" id="myForm" method="post" action="{{ url('saas/organization/industry/add') }}" enctype="multipart/form-data">
            @csrf
...
<script type="text/javascript">
    ...
    function updateMassItems() {
        ...
        if (arr.length == 0) {
            Swal.fire("Please select a master first !");
        } else if (status == 0) {
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            console.log(data);
                            if (data.success) {
                                Swal.fire('Deleted!', 'Industry deleted.', 'success');
                                var table = $('.industry-init').DataTable();
                                table.ajax.reload();
